...
Para o motor BLDC, a força contra-eletromotriz no referencial dq ($\mathbf{e}_{dq0}$) não é constante, mas sim dependente da posição do rotor devido à distribuição trapezoidal do fluxo. As componentes $e_d$ e $e_q$ são obtidas pela transformação direta das forças contra-eletromotrizes de fase $e_a, e_b, e_c$:

\begin{equation}
    e_d = \frac{2}{3} [e_a \cos(\theta_e) + e_b \cos(\theta_e - \frac{2\pi}{3}) + e_c \cos(\theta_e + \frac{2\pi}{3})]
\end{equation}

\begin{equation}
    e_q = \frac{2}{3} [-e_a \sin(\theta_e) - e_b \sin(\theta_e - \frac{2\pi}{3}) - e_c \sin(\theta_e + \frac{2\pi}{3})]
\end{equation}
