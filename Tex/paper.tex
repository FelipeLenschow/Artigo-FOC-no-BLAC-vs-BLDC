...
invariante em amplitude e a componente zero nula, a potência pode ser reescrita no referencial $dq0$:
...
Substituindo as equações de tensão $v_d$ e $v_q$ na equação de potência (\ref{eq:vdq_flux_opened} em \ref{eq:potencia_dq}):
...
Substituindo a expressão de $P_{em}$ encontrada anteriormente:

\begin{equation}
    T_e = \frac{3}{2} P \left[ \frac{e_d i_d + e_q i_q}{\omega_e} + (L_d - L_q) i_d i_q \right]
\end{equation}
...
\begin{equation}
    T_e = \underbrace{\frac{3}{2} P \frac{e_d i_d + e_q i_q}{\omega_e}}_{T_{sinc}} + \underbrace{\frac{3}{2} P (L_d - L_q) i_d i_q}_{T_{rel}}
\end{equation}
